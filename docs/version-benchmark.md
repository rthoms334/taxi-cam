...
This is not a live simulator frame-time measurement. Extra camera views remain the dominant intended cost when TAXI is on, including while parked: the live nose/tail image is the product.
...
Expected benefit, if any, is largest when cameras are parked after warmup and TAXI is off: PFD-only hook work is skipped, command-list lookups can hit a thread-local cache, and companion candidate rows refresh once per second. When TAXI is on at the default 15, MSFS still renders the two extra views. That cost is the live image, including ground movement at the gate. Closing those gates while parked would freeze the PFD. Lowering the rate to 5 or 10 can cut activation work while still showing motion; it is a setting, not an automatic gain.
...
```powershell
./tools/benchmark/Compare-TaxiCamVersions.ps1 -BaselineRef v0.9.8-build.38 -CandidateRef HEAD -RunIsolated
```
...
./tools/benchmark/Compare-TaxiCamVersions.ps1 -BaselineRef v0.9.8-build.38 -CandidateRef HEAD
```
