...
This is not a live simulator frame-time measurement. Extra camera views remain the dominant intended cost while taxiing. While parked with TAXI on, this tree now holds those gates closed after the first pair.

## Verdict

**Source changes that can reduce standing overhead exist, including a parked TAXI-on still-frame hold. A live FPS win versus 0.9.8 is unmeasured.**
