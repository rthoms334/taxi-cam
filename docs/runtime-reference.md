...
| **Allow Frame Generation (freeze risk)** | **Diagnostics**; off by default, saved in `settings.ini` (`[companion] allow_frame_generation`). While MSFS's own `UserCfg.opt` reports `FrameGeneration` other than `NONE`, the companion keeps the cameras off (no bridge load, or a Disconnect-style stop if they were on) and shows why; turning Frame Generation off in the simulator resumes Auto-connect. Turn this on only to test a build that addresses the frame-generation freeze. |
| **Restart as administrator** | Appears in the **Overview** connection box and the tray menu only while MSFS holds administrator rights that Taxi Cam lacks. Unsaved settings prompt first. Refusing the UAC prompt keeps the current instance running. |
...
| `single_camera` | 0 | Render only the nose for a performance test. While it is on and both cameras are requested, no composition can happen; the status line says so instead of waiting. |
...
| Frame Generation is on in MSFS graphics options | The companion read `FrameGeneration` from `UserCfg.opt` (Store: `%LOCALAPPDATA%\Packages\Microsoft.Limitless_8wekyb3d8bbwe\LocalCache`, Steam: `%APPDATA%\Microsoft Flight Simulator 2024`). On machines where the cameras freeze the simulator with frame generation (observed with RTX 50-series: all threads waiting, the DLSS-G helper thread blocked inside DXGI, no TDR), the cameras stay off until it is `NONE`. |
| A graphics hook (dxgi.dll/d3d12.dll, e.g. ReShade) is installed beside MSFS | Named on the Overview status line while the bridge reports zero captures. With ReShade 6.8 beside the executable, source recordings were invalidated continuously (`invalid_recordings` climbing with `reasons=0x1`, `tail=unknown_source_state`) and no capture completed; renaming its `dxgi.dll` restored capture on the same session. |
| First camera only is on (Diagnostics) | `single_camera=1` in the profile INI. The nose renders and is captured, the tail is never scheduled, and composition needs both feeds. Turn it off on Diagnostics and save. |
| MSFS is running as administrator (Windows 5) | MSFS was started elevated (for example **Run as administrator**) while Taxi Cam was not. Windows refuses the memory and thread rights the loader needs, so no retry can succeed. `launcher.log` records the elevated-simulator message with `error=5` at `phase=preflight`. Choose **Restart as administrator** in Overview or the tray menu: Taxi Cam exits and starts again through the UAC prompt with `--wait-for-exit <pid>`, then attaches to the same session. Starting MSFS normally avoids the prompt; an automatic exe.xml start inherits the simulator's rights and is unaffected. A generic **Cannot open MSFS in this Windows session** without the administrator wording is a different refusal. |
