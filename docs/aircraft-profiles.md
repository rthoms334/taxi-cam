...
The A350-900 nose defaults use the user's accepted live calibration: **right/up/forward 0 / −2 / 16.55 m, pitch/yaw −15° / 0°, lens 0.55 rad**. The tail uses **−18° / 0.62 rad** at its model-based mount position. A live simulator comparison with a cockpit ETACS reference confirmed the narrower tail view hides the horizon and brings the main gear lower in the pane. The A350 amber tail guides sit outside and below the observed bogies: their left upper point, lower corner and inner endpoint are `(0.29, 0.67)`, `(0.27, 0.84)` and `(0.36, 0.845)`, mirrored on the right. These remain fixed image references; changing camera settings can move the gear relative to them. A380 framing and guide positions are unchanged.

The A350-1000 uses the same nose calibration with forward position **20.36 m**, retaining the variant's 3.81 m offset, and a **−16.4° / 0.57 rad** tail preset derived from its longer camera-to-gear distance. This variant has **not been checked live** with these defaults. The geometry check projects the installed iniBuilds 1.2.6 `flight_model.cfg` gear contact points, while the -900 guide bounds also use observed simulator pixels. Neither replaces a cockpit comparison for the -1000. The Airbus planning diagram establishes display placement and guide colour for both variants, but does not establish identical perspective or field of view; passenger entertainment camera images can have a different crop. Existing saved mount settings take precedence over these defaults.
