...
The A350-900 defaults use the user's accepted calibration: nose **right/up/forward 0 / −2 / 16 m, pitch/yaw −15° / 0°, lens 0.55 rad**; tail **0 / 10 / −33 m, −15° / 0°, 0.62 rad**. Its amber tail guides use left upper point, lower corner and inner endpoint `(0.29, 0.76)`, `(0.27, 0.93)` and `(0.36, 0.935)`, mirrored on the right. The lower legs clear the main-gear contact and the earlier observed bogie bounds projected into this tail view. Final alignment needs a live check. These remain fixed image references; changing camera settings can move the gear relative to them. A380 framing and guide positions are unchanged.

The A350-1000 retains the same height, pitch, yaw and lens for each camera, with nose forward position **19.81 m** and tail **−36.17 m** for its longer fuselage. Its tail guides use `(0.31, 0.69)`, `(0.29, 0.85)` and `(0.37, 0.855)` because the main gear appears higher and nearer the image centre with the same lens. This variant has **not been checked live** with these defaults. The geometry check projects the installed iniBuilds 1.2.6 `flight_model.cfg` gear contact points. This does not replace a cockpit comparison for the -1000. The Airbus planning diagram establishes display placement and guide colour for both variants, but does not establish identical perspective or field of view; passenger entertainment camera images can have a different crop. Existing saved mount settings take precedence over these defaults.
